...
do isph = 1, ddx_data % params % nsph
    do i = 1, 3
        ...
        call solve(ddx_data, tol, esolv1, phi_cav, gradphi_cav, &
            & hessianphi_cav, psi, force, info)
        ddx_data % params % csph(i, isph) = ddx_data % params % csph(i, isph) - two*step
        call solve(ddx_data, tol, esolv2, phi_cav, gradphi_cav, &
            & hessianphi_cav, psi, force, info)
        ddx_data % params % csph(i, isph) = ddx_data % params % csph(i, isph) + step
...
subroutine solve(ddx_data, tol, esolv, phi_cav, gradphi_cav, hessianphi_cav, &
        & psi, force, info)
    ...
    call ddinit(ddx_data % params % nsph, ddx_data % params % charge, ddx_data % params % csph(1, :), &
        ...
        & ddx_data % params % jacobi_ndiis, ddx_data % params % nproc, ddx_data2, info)
